    ...
    def patch_device(module):
        ...
        for graph in graphs:
            for node in graph.findAllNodes("prim::Constant"):
                if "value" in node.attributeNames() and str(node.s("value") if node.kindOf("value") == "s" else "").startswith("cuda"):
            ...
            for graph in graphs:
                for node in graph.findAllNodes("aten::to"):
                    ...
                    for i in [1, 2]:  # dtype can be the second or third argument to aten::to()
                        if inputs[i].node().kindOf("value") == "i" and inputs[i].node().i("value") == 5:
