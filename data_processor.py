...
OUTPUT_PATH   = _ROOT / "data" / "processed"
...
MAX_VRAM_GB = 11.5    # total VRAM budget across all workers (leave 4.5 GB headroom)
...
def process_video(
    ...
    if len(raw_frames) < min_frames:
        logging.warning(
            ...
            len(raw_frames), min_frames, video_path.name,
        )
        return 0
...
def _worker_main(
    ...
    while True:
        ...
        try:
            ...
            logging.exception("Error procesando %s", video_path.name)
            vid_skipped += 1
